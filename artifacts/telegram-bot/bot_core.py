"""
bot_core.py — Shared bot logic (handlers, keyboards, data).
Used by both api/webhook.py (Vercel) and bot.py (local polling).
"""
...
# ─────────────────────────────────────────────
# 💾  PERSISTENT DATA
# Note: on Vercel (serverless) data.json resets on cold start.
# Set MINI_APP_URL in Vercel env vars for a permanent default.
# ─────────────────────────────────────────────
def load_data() -> dict:
    if DATA_FILE.exists():
        try:
